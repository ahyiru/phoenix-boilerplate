...
import {$clone,log,uniqueArr,isEqual} from './tools/yiru-tools';
...
// let rootDir=require('../node/config').rootDir;

// check version
const localYrui=require('yrui/package.json');
const localVer=localYrui.version;
// const gitUrl='https://raw.githubusercontent.com/ahyiru/phoenix-boilerplate/master/package.json';
...
export default class Router extends React.Component<RouterProps,RouterState>{
  ...
  static defaultProps={
    ...
    /*navbar:{
      dropList:null,
      showSearchbar:true,
      showRightTogbar:false,
      login:null,
      click:(v)=>{console.log(v);},
      listClick:(v)=>{console.log(v);},
    },
    sidebar:{
      projectList:null,
    ...
    rightbar:<h2>rightbar area</h2>,
    footer:<p>版权所有 &copy; 2017-2020 云创大前端</p>,
    main:{
      showPageHead:false,
      showPagetitle:false,
    },*/
  ...
  changeRouter=async ()=>{
    ...
    /*if(this.props.scroll){
      const ele:any=document.body;
      // const ele:any=this.refs.router;
      ele.data.scrollEle.style.top=ele.data.st=0;
      // ele.style.top=ele.data.st=0;
      ele.data.totop.style.display='none';
    }*/
    // let {data}=this.state;
    // let data=changeRoute(routers,browserRouter);
    /*this.setState({
      hasChange:false,
    });*/
    /*let n=Object.keys(this.refs)[0];
    let cr=this.refs[n];
    const Frame=this.props.frame; 
    const noMenu=data.current[0].noMenu;
    if(noMenu||!Frame){
      n='nomenu';
    }else{
      n='menu';
  ...
  /*addBase=()=>{
    let head=document.getElementsByTagName('head')[0],
        hasBase=head.getElementsByTagName('base');
    console.log(hasBase);
    if(hasBase.length<1){
  ...
  /*$scroll=()=>{
    ...
    if(show){
      this.totopbar.style.display='block';
    }else{
      this.totopbar.style.display='none';
    }
  };*/
  ...
  async componentDidMount(){
    // get version
    // await getVer();
    // this.routers=this.props.routers;
    // this.changeRouter();
    // this.popRouter();
  ...
  render() {
    // const Frame=this.props.frame;
    const {brand,navbar,sidebar,rightbar,main,footer,theme,browserRouter,homeUrl,horizontal}=this.props;
    if(horizontal){
